...
/*   Updated: 2024/07/19 20:26:10 by okrahl           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	**get_right_image(t_data *data, int ray_identification_number)
{
	...
	hit_x_int = (int)round(data->rays[ray_identification_number]->hit_x);
	hit_y_int = (int)round(data->rays[ray_identification_number]->hit_y);
...
void	update_color_row(t_data *data, int ray_identification_number, int start_y, int end_y)
{
	...
	image_height = get_image_height(right_image);
	image_width = get_image_width(right_image);
	if ((int)data->rays[ray_identification_number]->hit_x % data->settings->tile_size == 0)
		image_x = ((int)data->rays[ray_identification_number]->hit_y % data->settings->tile_size) * image_width / data->settings->tile_size;
	else
...
float	fix_fish_eye(t_data *data, int ray_identification_number)
{
	...
	ray_angle_rad = data->rays[ray_identification_number]->angle * M_PI / 180 ;
	diff_angle = ray_angle_rad - player_angle_rad;
	while (diff_angle < -M_PI)
		diff_angle += 2 * M_PI;
	while (diff_angle > M_PI)
		diff_angle -= 2 * M_PI;
	return (data->rays[ray_identification_number]->length * cos(diff_angle));
}
...
void	draw_3d_view(t_data *data)
{
	...
	{
		corrected_distance = fix_fish_eye(data, ray_identification_number);
		// data->rays[ray_identification_number]->length * cos((data->rays[ray_identification_number]->angle - data->player->player_direction) * M_PI / 180);
		//corrected_distance = data->rays[ray_identification_number]->length;
		...
		screen_x = (data->settings->window_width * ray_identification_number) / data->settings->num_rays;
		draw_wall_slice(data, screen_x, wall_height, ray_identification_number);
		if (ray_identification_number > 0)
		{
			prev_screen_x = (data->settings->window_width * (ray_identification_number - 1)) / data->settings->num_rays;
