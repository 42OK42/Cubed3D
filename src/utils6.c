...
int	no_grey_before_or_after_player(char **strarr, t_data *data)
{
	int	i;
	int	j;
		...
		while (strarr[i][j] != '\0')
		{
			...
			|| strarr[i][j] == 'E' || strarr[i][j] == 'W')
			{
				if (invalid_space(strarr, i, j))
					error_exit("player next to invalid space", data);
			}
			j++;
		}
		i++;
...
int	check__invalid_chars(char **strarr, t_data *data)
{
	int	i;
	int	j;
	...
	while (strarr[i] != NULL )
	{
		...
		while (strarr[i][j] != '\0')
		{
			if ((strarr[i][j] != '0' && strarr[i][j] != \
				...
				&& strarr[i][j] != ' ')
				error_exit("invalid chars in map", data);
			j++;
		}
		i++;
	}
...
int	check_chars(char **strarr, t_data *data)
{
	int	i;
	int	j;
	int	count;
	...
	if (count < 1)
		error_exit("Player not defined correctly", data);
	if (count > 1)
		error_exit("multiple players defined", data);
	return (1);
}
