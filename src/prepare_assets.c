...
/*   Updated: 2024/09/12 18:36:32 by okrahl           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize_assets(t_data *data)
{
	...
	data->assets->wall_south = load_xpm(data->assets->wall_south_path, data);
	data->assets->wall_north = load_xpm(data->assets->wall_north_path, data);
	data->assets->wall_east = load_xpm(data->assets->wall_east_path, data);
	data->assets->wall_west = load_xpm(data->assets->wall_west_path, data);
}

int	**load_xpm(char *PATH, t_data *data)
{
	int				**result;
	char			**xpm_lines;
	t_temp_assets	*temp;
...
char	**read_xpm(char *PATH, t_data *data)
{
	int		fd;
	char	*line;
	char	**xpm_lines;
	...
	if (fd == -1)
		error_exit("texture file not found", data);
	xpm_lines = (char **)ft_calloc(sizeof(char *), 19);
