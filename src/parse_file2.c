...
t_file_info	*initialize_file_info(void)
{
	...
	file_info = ft_calloc(sizeof(t_file_info), 1);
	if (!file_info)
		return (NULL);
	file_info->cc = -1;
	file_info->fc = -1;
...
int	is_element_color(char *file_content, t_data *data)
{
	if (*file_content == 'F')
	{
		if (data->file_info->fc != -1)
			error_exit("Floorcolor provided multiple times", data);
		return (1);
	}
	if (*file_content == 'C')
	{
		if (data->file_info->cc != -1)
			error_exit("Cellingcolor provided multiple times", data);
		return (1);
	}
	return (0);
}
...
int	is_element(char *file_content, t_data *data)
{
	if (*file_content == 'N')
	{
		if (data->file_info->path_no)
			error_exit("NO path provided multiple times", data);
		return (1);
	}
	if (*file_content == 'S')
	{
		if (data->file_info->path_su)
			error_exit("SU path provided multiple times", data);
		return (1);
	}
	if (*file_content == 'W')
	{
		if (data->file_info->path_we)
			error_exit("WE path provided multiple times", data);
		return (1);
	}
	if (*file_content == 'E')
	{
		if (data->file_info->path_ea)
			error_exit("EA path provided multiple times", data);
		return (1);
	}
