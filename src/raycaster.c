...
/*   Updated: 2024/09/16 19:34:55 by okrahl           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_ray(t_data *data, long double ray_angle, int i)
{
	...
	true_sect = NULL;
	j = 0;
	data->temp->start = ft_calloc(sizeof(t_point), 1);
...
int	check_wall_hit(t_data *data)
{
	...
	if ((northeast(data) && northeast_corner(data, cell_x, cell_y)) || \
		(northwest(data) && northwest_corner(data, cell_x, cell_y)) || \
	...
	if (data->map[cell_y][cell_x] == '1' || data->map[cell_y][cell_x] == ' ')
	{
		data->temp->hit_wall = 1;
		return (1);
	}
